...
export const ServiceStatusIndicator = () => {
  ...
  const [isRatingsAvailable, setIsRatingsAvailable] = useState(true)
  ...
  useEffect(() => {
    ...
    try {
      setIsRatingsAvailable(ratingsContext.isServiceAvailable)
    } catch (error) {
      console.warn("Servicio de calificaciones no inicializado correctamente")
      setIsRatingsAvailable(false)
    }
  ...
  }, [favoritesContext, reviewsContext, ratingsContext])
  ...
  if ((favoritesAvailable && isReviewsAvailable && isRatingsAvailable && isOnline) || !showIndicator) {
    return null
  }

  return (
    <div className="fixed bottom-4 right-4 z-50 bg-white dark:bg-gray-800 p-3 rounded-lg shadow-lg max-w-sm">
          ...
          <>
            ...
            <div className="flex flex-col">
              ...
              <div className="flex flex-col text-xs text-muted-foreground">
                ...
                {!isRatingsAvailable && (
                  <span>
                    • Calificaciones: No disponible - Ejecuta{" "}
                    <code className="bg-gray-100 px-1 py-0.5 rounded">node server.js</code> en
                    microservices/ratings-service
                  </span>
                )}
              </div>
            </div>
